/*
  Problem Name: Hartals
  Algorithm      : Math and DP
*/
...
void main()
{
  unsigned Sum_Hartals,Test_Case,Mult_Result,Range,Party_Number,Hartal_Difference[102],Register[4000],i,j,k;
  ...
  for (i=0;i<Test_Case;i++)
  {
    ...
    Sum_Hartals=0;
    for (j=1;j<=Range;j++) Register[j]=0;
    ...
    for (j=1;j<=Range;j++) Sum_Hartals+=Register[j];
    printf("%u\n",Sum_Hartals);
  }
}