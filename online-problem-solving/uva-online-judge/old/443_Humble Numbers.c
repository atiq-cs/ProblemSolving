/*
	...
	Algorithm: DP, Same as Ugly Numbers
*/
...
int main()
{
	...
	for(j = 0; j < i; j++)
	{
		...
		{
			if(humble[j] * 2 < humble[i]) humble[i] = humble[j] * 2;
		}
		else if(humble[j] * 3 > humble[i - 1])
		{
			if(humble[j] * 3 < humble[i]) humble[i] = humble[j] * 3;
		}
		else if(humble[j] * 5 > humble[i - 1])
		{
			if(humble[j] * 5 < humble[i]) humble[i] = humble[j] * 5;
		}
		else if(humble[j] * 7 > humble[i - 1])
		{
			if(humble[j] * 7 < humble[i]) humble[i] = humble[j] * 7;
		}
	}
	}
